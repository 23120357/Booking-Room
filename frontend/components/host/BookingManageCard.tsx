          ...
          <div className="flex items-center gap-2 text-booking-muted">
            {listing.status === 'rented' && (
              <button
                type="button"
                disabled={toggling}
                onClick={() => onToggleVisibility?.(listing.id, false)}
                title="Đánh dấu phòng trống (Tạm ẩn tin đăng)"
                className="flex h-9 w-9 items-center justify-center rounded-lg transition hover:bg-[#f3f3fe] text-emerald-600 hover:text-emerald-700 disabled:opacity-60 disabled:cursor-wait"
              >
